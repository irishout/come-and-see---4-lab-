...
async def movie_search(message: Message, state: FSMContext) -> None:
    ...
    await state.set_state(MovieSearch.waiting_for_smth)

#отображение фильма
@router.message(MovieSearch.waiting_for_smth)
    ...
    if movie_title != "Вернуться в меню":
        try:
            ...
            await message.answer_photo(photo,caption= 
                                    ...
                                    f'             filmCritics: {raitingfilmCritics}\n'
                                    '\n'
                                    f'📆Год: {year}', reply_markup=kb.exit
                                        )
        except Exception as e:
